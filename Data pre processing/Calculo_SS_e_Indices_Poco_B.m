...
plot_MapPCA = 0;                                                 %fig2
PCA_normaliza = 0;                                               %fig2
PCA_normaliza_padrao = 0;                                        %fig2
PCA_normaliza_log = 0;
...
tempo_inicio_perf = 35;    % PocoB run1
tempo_fim_perf = 60;       % PocoB run1
% tempo_inicio_perf = 5;    % PocoB run2
% tempo_fim_perf = 91;       % PocoB run2
% tempo_inicio_perf = 11;    % PocoB run3
% tempo_fim_perf = 180;       % PocoB run3
...
%   ----------------   >>>   DEFINE VETORES:
%   ---   LAS
BitOnBot_GERAL = LAS_OBTM;
SUP_ElapsedTime = LAS_ElapsedTime;
SUP_RPM = LAS_RPM;
SUP_STOR = LAS_STOR;
SUP_SWOB = LAS_SWOB;
SUP_DEPTH = LAS_BIT_DEPTH;
SUP_TFLO = LAS_TFLO;
SUP_VIB_TOR = LAS_VIB_TOR;

SUP_GR_CAL = LAS_GR_CAL;    % Calibrated Gamma Ray
% SUP_ECD = LAS_ECD;       % Equivalent Circulating Density
SUP_SPPA = LAS_SPPA;      % Standpipe Pressure 

%   ---   DOWN
DOWN_ElapsedTime_sync = BHA_ElapsedTime_sync;
DOWN_RPMMax = BHA_BBHD_RPMMax;
DOWN_RPMMin = BHA_BBHD_RPMMin;
DOWN_RPMMean = BHA_BBHD_RPMMean;
...
cuts_all = cuts_all(inicio:final-1,:);
...
if plot_MapPCA == 1
    ...
    for i=1:size(DIMENSOES, 2)
        ...
        DIMENCOES_n(:,i) = normalizado;
    end
    
    
    [coeff_n,score_n,latent_n,tsquared_n,explained_n,mu_n] = pca(DIMENCOES_n); % , 'Rows','pairwise');
...
Xt_PocoB = DIMENSOES;
Xt_PocoB_SS = SS_RPM_DOWN_SUP;



Xt_PocoBr1_50_100_50mm = [Xt_PocoB, Xt_PocoB_SS];
